		...
		messages := []InAppMessage{
			{
				ID:          "welcome_2025",
				Type:        "info",
				Title:       "🎊 Welcome to 2D Expect 2025!",
				Message:     "Thank you for using 2D Expect! We're committed to providing you with the best lottery experience. Check out our new features and enjoy real-time updates!",
				ImageURL:    "",
				ActionText:  "",
				ActionURL:   "",
				Priority:    5,
			...
			},
			{
				...
				ActionURL:   "https://play.google.com/store/apps/details?id=com.twod.expect",
				Priority:    10,
				StartDate:   "2025-11-01",
				EndDate:     "2025-11-30",
				ShowOnce:    false,
...
func main() {
	...
	r.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{
			"name":     "2D Expect - App Config Server",
			"version":  "1.0.0",
			"status":   "running",
			"database": "SQLite3",
			"endpoints": []string{
				"GET  /api/2dexpect/config   - Get full app configuration",
				"GET  /api/2dexpect/version  - Get version info only",
				"GET  /api/2dexpect/messages - Get in-app messages only",
				"POST /api/2dexpect/config   - Update configuration (admin)",
